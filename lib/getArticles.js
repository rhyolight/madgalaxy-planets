...
function getMostRecentArticles(pageNum, callback) {
    request('http://localhost:3000/api/v1/articles?pageNum=' + pageNum, function(error, response, body) {
        console.log('making a request for: ' + 'http://localhost:3000/api/v1/articles?pageNum=' + pageNum);
...
function getArticlesByID(pageNum, id, callback) {
    request('http://localhost:3000/api/v1/articles/id/' + id, function(error, response, body) {
        console.log('making a request for: ' + 'http://localhost:3000/api/v1/articles/id/' + id);
        if (!error && response.statusCode == 200) {
            callback(error, JSON.parse(body));
        }
    });
...
function getArticlesByTag(pageNum, tag, callback) {
    request('http://localhost:3000/api/v1/articles/tags/' + tag + '?pageNum=' + pageNum, function(error, response, body) {
        console.log('making a request for: ' + 'http://localhost:3000/api/v1/articles/tags/' + tag + '?pageNum=' + pageNum);
        if (!error && response.statusCode == 200) {
            callback(error, JSON.parse(body));
        }
    });
...
function getArticlesBySearch(pageNum, searchParam, callback) {
    request('http://localhost:3000/api/v1/search/?searchString=' + searchParam + '&searchTag=' + planetSpecificData.feed_tag + '&pageNum=' + pageNum, function(error, response, body) {
        console.log('making a request for: ' + 'http://localhost:3000/api/v1/search/?searchString=' + searchParam + '&searchTag=' + planetSpecificData.feed_tag + '&pageNum=' + pageNum);
        if (!error && response.statusCode == 200) {
            callback(error, JSON.parse(body));
        }
    });
