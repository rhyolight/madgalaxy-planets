...
function increaseViewCount(id) {
    request('http://madgalaxy.herokuapp.com/api/v1/articles/id/' + id + '/upview', function(error, response, body) {
        console.log('making a request for: ' + 'http://madgalaxy.herokuapp.com/api/v1/articles/id/' + id + '/upview');
        if (!error && response.statusCode == 200) {
            
        }
        else{
            console.log('error increasing viewcount of article: ' + id);
        }
    });
}
