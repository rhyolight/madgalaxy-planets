...
function getArticlesByTag(pageNum, tag, callback){
    request('http://madgalaxy.herokuapp.com/api/v1/articles/', function (error, response, body) {
      console.log('request for: ' + 'http://madgalaxy.herokuapp.com/api/v1/articles/tags/' + tag + '?pageNum=' + pageNum);
      console.log(error);
      if (!error && response.statusCode == 200) {
        callback(error, JSON.parse(body));
      }
...
function getArticlesBySearch(pageNum, searchParam, callback){
    request('http://madgalaxy.herokuapp.com/api/v1/search/?searchString=' + searchParam + '&pageNum=' + pageNum, function (error, response, body) {
      if (!error && response.statusCode == 200) {
        callback(error, JSON.parse(body));
      }
    });
