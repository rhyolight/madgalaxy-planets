...
function getArticlesByID(pageNum, id, callback){
    request('http://www.madgalaxy.io/api/v1/articles/id/' + id + '?pageNum='+ pageNum, function (error, response, body) {
      if (!error && response.statusCode == 200) {
        callback(error, JSON.parse(body));
      }
    });
...
function getArticlesBySearch(pageNum, searchParam, callback){
    request('http://www.madgalaxy.io/api/v1/search/?searchString=' + searchParam + '&pageNum=' + pageNum, function (error, response, body) {
      if (!error && response.statusCode == 200) {
        callback(error, JSON.parse(body));
      }
    });
